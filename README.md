...
A forensic, panel-driven UK property dashboard: **postcode** or **UPRN** search, EPC-led identity, Land Registry recent sales, optional UKHPI context, Ofsted nearby providers (scraped), map, quick nearby-postcode shortcuts, and placeholder modules for transport, utilities, and risk.
...
6. `**renderAllPanels(state)`** paints all panels (including nearby postcode shortcut chips), then `**renderFooterContext(state)`** (`js/panels/footer-context.js`) fills the **Notes & data context** block at the bottom of the page (tenure note + value, HPI/PPD references, per-sale HPI maths text + equation lines, Ofsted search notes) so main panels stay data-focused.
...
| Row | Panels                                                                                                                                                                                                                                                                                      |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Registered Asset** (`panel-identity`) — EPC-led identity: title address (with postcode), UPRN, **last sale** (matched Land Registry row when available), **HPI projection** (matched-sale HPI-adjusted value + market-average adjusted £/ft² × subject area), ratings, **floor area m² + ft²**, **Date of EPC certificate** (inspection date if present, else lodgement), £/m² if transaction price exists. Last-sale/HPI matching uses weighted address similarity (including house-number signal) rather than strict string equality. |
...
| 3   | **Schools** (`panel-schools`) — Ofsted search results (name link, category, rating, distance, last report). UI shows **3 rows by default** and expands to **10** via `...see more...` (Ofsted `meta.note`: **footer** / `footer-context.js`)                                                                                                                            |
...
| 5   | **Nearby Postcodes** (`panel-nearby-postcodes`) — up to 4 nearest postcode chips from postcodes.io (`lat/lon`), deduped and excluding the active postcode; clicking a chip fills postcode input, clears UPRN, and re-runs resolve. |
...
| *(client-side)* postcodes.io `/postcodes?lat=&lon=` | Nearby postcode shortcuts (not served by Worker; called directly from frontend) |
...
js/
  ...
  normalisers/
  panels/      # panel1–8 + footer-context.js (long-form context for footer)
  utils/
app.js
index.html     # header (search) scrolls; sticky strip = address select only; main panels incl. nearby postcode shortcuts; footer = Notes & data context
```
...
This repo is a **build-less SPA** + **one Worker**: EPC-centric resolve, Land Registry SPARQL for PPI (and UKHPI attempt), Ofsted HTML for schools, Leaflet map, and **four placeholder GET routes** for transport/utilities/risk data. `**docs/DEVELOPMENT.md`** is the handoff file for agents and humans; keep it in sync when you change Worker routes or normalisers. **Agent rule:** when behaviour/contracts/orchestration change, update `AGENTS.md`, this `README.md`, and `docs/DEVELOPMENT.md` together.